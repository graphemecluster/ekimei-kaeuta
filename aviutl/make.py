...
mask = os.path.abspath("../垰瀬内新雛形地図マスク4.png")

Tk().withdraw()
utaus = []
for filename in filedialog.askopenfilenames(
    title="UTAU ファイルを開く（複数選択可）", filetypes=[("UTAU Sequence Text", "*.ust"), ("すべてのファイル", "*.*")], defaultextension=".ust"
):
...
filename = filedialog.askopenfilename(
    title="CSV ファイルを開く", filetypes=[("Comma-Separated Values", "*.csv"), ("すべてのファイル", "*.*")], defaultextension=".csv"
)
if not filename:
    exit()
...
with open(filename, newline="") as file:
    for item in csv.reader(file):
        ...
        name = item[0]
        if name[-1] == ")":
            name = name.replace("(", "<s80> (")
        elif name[-1] == "]":
            name = name[:name.index("[")]
        assert item[2] in prefs, f"#{len(data)} {name.replace('<s80> ', '')}（{item[1]}）：存在しない県を検出しました：{item[2]}"
        data += [(name, item[1], item[2], item[5] + " " + item[6]
                  if item[6] else item[5], item[4].rjust(3, "0"))]
...
def map_num(index):
    return data[index - 1][-1] if index else "000"
...
def write_text(index, start, end):
    ...
    output["%d.0" % j] = {
        ...
        "align": 1,  # 中央揃え[中]
        "font": "ヒラギノ角ゴ ProN W3",
        "text": tohex(template % data[index][:-1]),
    }
    output.add_section("%d.1" % j)
    output["%d.1" % j] = {
        "_name": "標準描画",
    ...
    while highest > lowest:
        ...
        while count[nokori][1] == highest:
            nokori += 1
        if nokori >= num_prefs - 10 and (highest not in prev_yokoku or prev_yokoku[highest] != nokori):
            append = ""
            if nokori >= num_prefs - 4:
                append = f"({'･'.join(map(pref_abbr, next(zip(*count[nokori:]))))})"
                if nokori == num_prefs - 4:
                    append = f"<s40>{append}<s>"
            yokoku += f"<#{colors[highest]}>{highest}周制覇まで残り{num_prefs - nokori}県{append}\r\n"
...
def write_kansou(start, end):
    ...
    while highest > lowest:
        while count[nokori][1] == highest:
            nokori += 1
        yokoku += f"<#{colors[highest]}>■全県{highest}周制覇まで残り{num_prefs - nokori}県\r\n"
        if nokori >= num_prefs - 4:
            yokoku += f"  ({'･'.join(map(pref_abbr, next(zip(*count[nokori:]))))})\r\n"
        highest -= 1
    while highest:
    ...
    while c < len(reading):
        ...
        if norm in "ーっん":
            ...
            try:
                ...
                raise ValueError(
                    f"#{index} {item[0].replace('<s80> ', '')}（{reading}）：該当しない文字を検出しました：{reading[c]}")
            else:
                while True:
                    try:
                        assert reading[c] != " "
...
if len(utaus) > 1:
    lyrics = sorted(lyrics, key=lambda item: item["start"])
    ...
    while li < len(lyrics) and si < len(stations):
        ...
        if si:
            ...
            if skippable(stations[si - 1], stations[si]):
                ...
                continue
        raise ValueError(
            f"#{index} {data[index][0]}（{data[index][1]}）：歌詞と駅名が一致しません：{lyrics[li]['mora']} vs {stations[si]['mora']}")
    if si and index != stations[si]["index"]:
        write_text(index, start, lyrics[li - 1]["end"])
        start = lyrics[li]["start"]
        index += 1
        assert index == stations[si]["index"], f"#{index} {data[index][0]}（{data[index][1]}）：駅が飛ばされました"
    li += 1
    si += 1
write_text(index, start, lyrics[li - 1]["end"])
...
if li < len(lyrics) and messagebox.askquestion("確認", "歌詞が余っています。それでも続行しますか？") == 'no' or \
        si < len(stations) and messagebox.askquestion("確認", "駅名が余っています。それでも続行しますか？") == 'no':
    exit()

filename = filedialog.asksaveasfilename(
    title="EXO ファイルを保存", filetypes=[("AviUtl External Object", "*.exo"), ("すべてのファイル", "*.*")], defaultextension=".exo"
)
if not filename:
    exit()
